...
public class TextEditingApp : App {
    ...
    private IEnumerator Start() {
        ...
        float initialWaitingDelayForImageAnimation = 1.5f;
    ...
    private IEnumerator ActivateAndFillTopicButtonsIfEmptyCoroutine() {
        ...
        float initialWaitingTimeForHeaderTextAnimation = 1f;
    ...
    private IEnumerator ActivateAllegationButtonsCoroutine() {
        float initialWaitingTimeForHeaderTextAnimation = 1.5f;
        yield return new WaitForSeconds(initialWaitingTimeForHeaderTextAnimation);
        foreach (Button button in allegationButtons) {
            button.gameObject.SetActive(true);
            button.interactable = false;
            ...
            if (!doTopicButtonsHaveListeners) {
                button.onClick.AddListener(() => {
                    ...
                    StartCoroutine(CheckCorrectnessCounterAndOpenCanvasAgainIfPossible());
                });
            }
        }
    ...
    private IEnumerator CheckCorrectnessCounterAndOpenCanvasAgainIfPossible() {
    ...
    public void SendTweetAndCloseTheApp() {
        // This function needs to be called from the SendTweetButton
        StartCoroutine(SendTweetAndCloseTheAppAndOpenCanvas());
    }

    [SerializeField] private TextEditingApp textEditingApp;

    private IEnumerator SendTweetAndCloseTheAppAndOpenCanvas() {
        yield return new WaitForSeconds(3f);
